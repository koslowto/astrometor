...
slider_e = Slider(ax_slider_e, "e", valmin = 0.1, valmax = 0.9, valinit = 0.5, valstep = 0.01, orientation='vertical')

# Slider to cotrol the inclination
ax_slider_i = plt.axes([0.825, 0.25, 0.05, 0.6])
slider_i = Slider(ax_slider_i, "i", valmin = 0, valmax = 360, valinit = 40, valstep = 1, orientation='vertical')
...
fig.canvas.mpl_connect('button_release_event', lambda event:
    graph.update_graph(mode=check.value_selected, e=slider_e.val, w=slider_w.val, i=slider_i.val, o=slider_o.val, v=slider_v.val, ax=ax))
